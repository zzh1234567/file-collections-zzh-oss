...
export default class OSSStore extends StorageAdapter {
  constructor({
  ...
  _fileKeyMaker(fileRecord) {
    ...
    debug("OSSStore _fileKeyMaker result:", result);
  ...
  _getReadStream(fileKey, { start: startPos, end: endPos } = {}) {
    ...
    debug("OSSStore _getReadStream opts:", opts);
  ...
  _getWriteStream(fileKey, options = {}) {
    ...
    debug("OSSStore _getWriteStream opts:", opts);
  ...
  _removeFile(fileKey) {
    debug("OSSStore _removeFile called for fileKey", fileKey);
